...
public class BSTImpl<T extends Comparable<T>> implements BST<T> {
	...
	private void removeLeftNode(BTNode<T> node) {
	...
	private void removeRoot(BTNode<T> node) {
		// APENAS FILHO A ESQUERDA
		if (node.getRight().isEmpty()) {
	...
	private int getLeafs(BTNode<T> node) {
		if (!node.isEmpty()) {
			if (node.isLeaf()) {
				return 1;
			} else {
				return getLeafs(node.getLeft()) + getLeafs(node.getRight());
			}
		}
		return 0;
	}

	public int getSinglesNodes() {
		return getSinglesNodes(this.root);
	}

	private int getSinglesNodes(BTNode<T> node) {
		if (!node.isEmpty()) {
			if (singleChild(node)) {
				return 1 + getSinglesNodes(node.getLeft()) + getSinglesNodes(node.getRight());
			} else {
				return getSinglesNodes(node.getLeft()) + getSinglesNodes(node.getRight());
			}
		}
		return 0;
	}

	public int getNodesHeight2() {
		return getNodesHeight2(this.root);
	}

	private int getNodesHeight2(BTNode<T> node) {
		if (!node.isEmpty()) {
			if (hasTwoNodes(node)) {
				return 1 + getNodesHeight2(node.getLeft()) + getNodesHeight2(node.getRight());
			} else {
				return getNodesHeight2(node.getLeft()) + getNodesHeight2(node.getRight());
			}
		}
		return 0;
	}

	private boolean hasTwoNodes(BTNode<T> node) {
		if (node.isEmpty()) {
			return false;
		} else {
